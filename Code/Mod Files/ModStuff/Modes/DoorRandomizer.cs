...
namespace ModStuff
{
     public class DoorsRandomizer : Singleton<DoorsRandomizer>
     {
          ...
          void BuildVanillaRooms()
          {
               ...
               {
                new RoomNode("FluffyFields", new string[] {"CaveN", "FF_SW3", "CaveQ", "CaveH", "CaveC", "CaveB", "FF_SS1", "CaveL", "FF_SW1", "CaveA", "CaveU" , "PillowFortOutside", "CaveE", "CaveS", "CaveO", "CaveM", "CaveX", "FF_VH1", "CaveR", "CaveD", "FF_FR4", "CaveJ", "FF_FR2", "FF_FR1", "FF_SW4", "CaveP", "CaveF", "CaveG", "CaveK", "CaveH2", "FF_CC1", "CaveY", "CaveW", "CaveI"}),
                ...
                new RoomNode("StarWoods", new string[] {"CaveG", "CaveI", "CaveR1", "SW_CC1", "CaveR2", "SW_FF4", "CaveQ2", "CaveS1", "SW_FF3", "TrashCaveOutside", "CaveA", "CaveF", "CaveE", "CaveC", "SW_FR1", "CaveB", "CaveN", "SW_CC2", "CaveP", "CaveD", "CaveH", "CaveQ1", "CaveJ", "CaveK", "SW_FF1"}),
          ...
          string PrintCheatSheet()
          {
               ...
               for (int k = 0; k < list.Count; k++)
               {
                    ...
                    doors.Add(sceneRemoved + " -> " + randomizedDoors[list[k]].scene + "/" + randomizedDoors[list[k]].door);
               }
               output += PrintSceneDoors(scene, doors.ToArray());
          ...
          public void RandomizeWithSeed(string seed, bool printSheet)
          {
               currentSeed = seed;
               ...
               if (printSheet)
               {
                    ...
                    File.WriteAllText(ModMaster.RandomizerPath + "Door randomizer " + currentSeed + ".txt", output);
               }
          ...
          //Fix buffered seed
          public string FixSeed(string seed)
          {
               ...
               for (int i = 0; i < SeedLength; i++)
               {
                    if (i < upperSeed.Length && seedChars.Contains(upperSeed[i]))
                    {
                         output += upperSeed[i];
                    }
                    else
          ...
          public void CheckForRandomization()
          {
               bool useRandom = false;
               string randomizerDoors = ModSaver.LoadStringFromFile("mod", "randomizer_doors");
               if (!string.IsNullOrEmpty(randomizerDoors) && randomizerDoors == "1") { useRandom = true; }
               if (useRandom)
               {
                    string savedSeed = ModSaver.LoadStringFromFile("mod", "randomizer_doorsseed");
                    if (currentSeed != savedSeed || !useRandomized)
                    {
                         RandomizeWithSeed(savedSeed, false);
                    }
                    else
                    {
                         useRandomized = true;
